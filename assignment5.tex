...
\begin{align}
\mu_x&=\mu_y=0 \label{mu}\\
\sigma_x&=1 \label{sig1}\\
\sigma_y&= \frac{5}{8} \label{sig2}\\
\rho&=\frac{3}{5} \label{rho}
\end{align}
We need to find $E(XY)$
\begin{align}
E(XY)&= \int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty} xyf_{X,Y}(x,y)\mathrm{d}y \mathrm{d}x \label{2}
\end{align}
Substiting \eqref{eq} in \eqref{2} and simplifing ig,we get
\begin{align}
E(XY)&=\rho \sigma_x\sigma_y+\mu_x\mu_y \label{eq:3}
\end{align}
Substituting \eqref{mu},\eqref{sig1},\eqref{sig2},\eqref{rho} in \eqref{eq:3} we get
\begin{align}
